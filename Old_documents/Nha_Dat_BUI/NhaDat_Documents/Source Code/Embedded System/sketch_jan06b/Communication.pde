...
void serial2PrintFloatArr(float * arr, uint8 length) 
{
  for(uint8 i=0; i<length; i++)
  {
    serial2FloatPrint(arr[i]);
    Serial2.print(",");
  }
}
void serialUSBPrintFloatArr(float * arr, uint8 length) 
{
  for(uint8 i=0; i<length; i++)
  {
    serialUSBFloatPrint(arr[i]);
    SerialUSB.print(",");
  }
}
...
void serialUSBFloatPrint(float f) 
{
  uint8 *b = (uint8 *)&f;
  for(uint8 i=0; i<4; i++) 
  {
...
void serial2FloatPrint(float f) 
{
  uint8 *b = (uint8 *)&f;
  for(uint8 i=0; i<4; i++) 
  {
