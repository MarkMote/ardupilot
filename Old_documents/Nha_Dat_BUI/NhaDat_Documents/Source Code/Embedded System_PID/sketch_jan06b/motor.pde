...
//Maximum value for motorData will be 215.75 rad/s (Ana's report page 40)

void controlMotors(const float *motorData)    
{
  uint16 PWMData[4] = {0,0,0,0};
  uint8 i;
  
  //Convert from rad/s to rpm and then to average Voltage
  float Pi;
  Pi=3.1415926536;
  //float rpm[4]={0,0,0,0};
  ...
  for (i=0;i<4;i++)
  {
    if(motorData[i] <= 0)  averageVoltage[i] = 0;  //rpm[i]=0;
      else if(motorData[i]  >= 216) averageVoltage[i] = (216*60/Pi)/(1150);// Maximum rad/s to maximum rpm required for the quadricopter
        else  averageVoltage[i] = float (motorData[i]*60/Pi)/(1150);
  }    

  //Convert from average Voltage to percentage and then to PWMdata
  
  for(i=0;i<4;i++)
  {
    if(motorData[i] <= 0)  PWMData[i] = 0;  //PWM占空比调整到0%，接口常低电平。
      else if(motorData[i]  >= 216) PWMData[i] = (uint16) ((216*60/Pi)/(1150)/(11.1)*65535) ;  //PWM占空比100%，接口常高电平。
        else  PWMData[i] = (uint16) (averageVoltage[i]/(11.1)*65535);
  }     
...
/*   SerialUSB.println(PWMData[0]);
  SerialUSB.println(PWMData[1]);
  SerialUSB.println(PWMData[2]);
  SerialUSB.println(PWMData[3]);  */ 
