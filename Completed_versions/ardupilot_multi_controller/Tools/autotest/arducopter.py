...
FRAME='+'
TARGET='sitl'
HOME=mavutil.location(-35.362938,149.165085,0,270)
...
def Test_mode(mavproxy,mav):
    ...
    # Fly mission #1
    print("# Load copter_mission")
    if not load_mission_from_file(mavproxy, mav, os.path.join(testdir, "Pitch_Roll_Test7.txt")):
        print("load copter_mission failed")
        return False
    ...
    mavproxy.send('switch 4\n') # auto mode
    wait_mode(mav, 'AUTO')
    ret = wait_waypoint(mav, 0, num_wp-1, timeout=50000, mode='AUTO')
    expect_msg = "Reached Command #%u" % (num_wp-1)
    if (ret):
        mavproxy.expect(expect_msg)
    print("test: MISSION COMPLETE: passed=%s" % ret)
