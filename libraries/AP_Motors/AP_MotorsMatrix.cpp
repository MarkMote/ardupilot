...
void AP_MotorsMatrix::output_armed_new(double *input)
{
    ...
    ///Try to stabilize the quad in pitch, roll and yaw mode (This code is specific for quad +, need to change when change the frame)
        //Yaw
    /*if (fabs(output_signal[1]-output_signal[3])>100.0)
        if (output_signal[1]>output_signal[3])
            output_signal[3] = output_signal[1] - 100.0;
        else output_signal[1] = output_signal[3] -100.0;*/  
    // Roll
    /*if (fabs(output_signal[0]-output_signal[1])>25.0)
        if (output_signal[0]>output_signal[1])
            output_signal[0] = output_signal[1] + 25.0;
        else output_signal[0] = output_signal[1] -25.0;
        
    // Pitch
    if (fabs(output_signal[2]-output_signal[3])>25.0)
        if (output_signal[2]>output_signal[3])
            output_signal[2] = output_signal[3] + 25.0;
        else output_signal[2] = output_signal[3] -25.0;*/ 
