...
#define millis2 libmaple_millis2
#define micros2 libmaple_micros2
...
{    
    uint32_t start = libmaple_micros2();
    
    while (ms > 0) {
        while ((libmaple_micros2() - start) >= 1000) {
            ms--;
            if (ms == 0) break;
            start += 1000;
        }
...
uint32_t FLYMAPLEScheduler::millis() {
    return libmaple_millis2();
}

uint32_t FLYMAPLEScheduler::micros() {
    return libmaple_micros2();
}
